...
class Program
{
    ...
    static void StudentGrade()
    {
        ...
        Console.WriteLine("Enter homework weight (between 0-1)");
        double hperc = double.Parse(Console.ReadLine());
        Console.WriteLine("Enter participation weight (between 0-1)");
        double pperc = double.Parse(Console.ReadLine());
        Console.WriteLine("Enter quizzes weight (between 0-1)");
        double qperc = double.Parse(Console.ReadLine());
        Console.WriteLine("Enter midterm weight (between 0-1)");
        double mperc = double.Parse(Console.ReadLine());
        Console.WriteLine("Enter final weight (between 0-1)");
        double fperc = double.Parse(Console.ReadLine());
        double totalgrade = (((hgrade * hperc) + (pgrade * pperc) + (qgrade * qperc) + (mgrade * mperc) + (fgrade * fperc)));
        Console.WriteLine("\n"+firstname+" "+lastname+" ("+studentid+"): "+"Your final grade is " + totalgrade + "%");
        Console.WriteLine("\nPress any key to return to menu...");
        Console.ReadKey();
    }
}